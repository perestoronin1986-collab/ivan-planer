...
- **fix(inbox):** первая же надиктованная фраза легла в инбокс как 508 символов нарастающих повторов. `onresult` дописывал каждый результат с `isFinal`, а движок присылает не фрагмент, а всю фразу заново на каждом уточнении — включая обрывки слов.
- **Решение:** `collectTranscript()` собирает текст из `event.results` целиком и отдаёт состояние, подписчик заменяет значение. Текст завершённых сессий копится в `committedRef` — при перезапуске сессии `results` обнуляется.
- **Регрессия:** `npm run check:speech` (`scripts/check-speech.ts`) на потоке событий, снятом с реальной записи. Тестового фреймворка в проекте нет — скрипт самостоятельный, как `planer.ts`.
- Грабли — [[ERRORS]], 18.09.
