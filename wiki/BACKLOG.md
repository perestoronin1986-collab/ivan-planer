...
- **Досрочное завершение регулярки (2026-08-02)** — иконка «стоп» в строке `/recurring`: текущее вхождение закрывается как выполненное, остальные открытые вхождения серии удаляются (мягко, `deleted_at`), шаблону проставляется `rrule_until`. Всё одной Dexie-транзакцией, мутация `endRecurringSeriesLocal`. Кнопка только на `/recurring` — через опциональный проп `onEndSeries` у общего `TaskItem`. Механика подтверждения вынесена в `ConfirmActionButton`, `ConfirmDeleteButton` остался обёрткой. Заодно починен `processOverdueTasks` (не фильтровал `deleted_at`). Возобновления серии нет. Подробности — [[Features/recurring-end-series]].
- **Push-уведомления (2026-05-27)** — VAPID + Web Push API. `task.remind_at` → Postgres trigger `sync_task_notification` создаёт строку в `notification`. Крон `/api/cron/push` каждые 5 мин (внешний cron-job.org — Vercel Hobby не даёт чаще раза в сутки, а GitHub Actions с 07.08 только резерв раз в час) читает pending notifications и шлёт через `web-push`. SW обрабатывает `push` event. Подписка/отписка через `/api/push/subscribe`+`/unsubscribe`. UI: тоггл в `/settings`. `RemindAtPicker` в форме задачи — пресеты «10 мин / 1 час / 1 день в 9:00 / точно» + datetime-local.
